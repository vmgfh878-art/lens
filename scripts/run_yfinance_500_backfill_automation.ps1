param(
    ...
    [int]$ChunkSize = 50,
    [int]$MaxChunksPerRun = 2,
    [double]$SleepSecondsBetweenTickers = 0.2,
    [double]$SleepSecondsBetweenChunks = 60,
    [string]$RunDir = "docs\yfinance_500_backfill_runs",
    [switch]$DryRun,
    [switch]$Apply
)
...
if (-not $DryRun -and -not $Apply) {
    throw "이 자동화는 백필 목적이므로 -Apply 또는 -DryRun을 명시해야 합니다."
}
...
$MetricsPath = Join-Path $RunDirPath "yfinance_500_backfill_metrics_${SafeDate}_${RunStamp}.json"
$ReportPath = Join-Path $RunDirPath "yfinance_500_backfill_report_${SafeDate}_${RunStamp}.md"
$StdoutPath = Join-Path $RunDirPath "yfinance_500_backfill_stdout_${SafeDate}_${RunStamp}.log"
$StderrPath = Join-Path $RunDirPath "yfinance_500_backfill_stderr_${SafeDate}_${RunStamp}.log"
$FailedCsvPath = Join-Path $RunDirPath "yfinance_500_backfill_failed_tickers_${SafeDate}_${RunStamp}.csv"
$DistributionCsvPath = Join-Path $RunDirPath "yfinance_500_backfill_latest_distribution_${SafeDate}_${RunStamp}.csv"
$LatestMetricsPath = Join-Path $Root "docs\yfinance_500_backfill_metrics.json"
$LatestReportPath = Join-Path $Root "docs\yfinance_500_backfill_latest.md"
$HistoryPath = Join-Path $RunDirPath "yfinance_500_backfill_history.csv"
...
if ($null -eq $Metrics) {
    $Metrics = [pscustomobject]@{
        ...
        final_summary = "yfinance 500 backfill metrics file was not created."
...
$ReportLines = @(
    "# yfinance 500 backfill automation result",
    "",
    "- run_at: $RunAtText",
    "- run_date: $RunDate",
    "- mode: $ModeText",
