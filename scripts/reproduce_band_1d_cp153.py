"""
Lens 운영 1D 밴드 (CP153 TiDE q15/q85 lower_focused) 재현 wrapper.

사용:
    python scripts/reproduce_band_1d_cp153.py --external ./external_package
    python scripts/reproduce_band_1d_cp153.py --external ./external_package --verify-only

요구:
    - Python 3.10.x
    - torch 2.11.0+cu128 + GPU sm_120 (RTX 5060 Ti)
    - external_package 폴더에 checkpoint + 학습 시점 parquet 동봉
...
REQUIRED_CHECKPOINTS = [
    "cp153/tide-1D-ea54dcae654d-seed_7.pt",
    "cp153/tide-1D-ea54dcae654d-seed_42.pt",
    "cp153/tide-1D-ea54dcae654d-seed_123.pt",
]
SERVING_PARQUET = REPO_ROOT / "backend" / "data" / "v1" / "predictions_band_1d.parquet"
...
def stage_files(external: Path) -> None:
    ...
    ckpt_dst = REPO_ROOT / "data" / "artifacts" / "cp153"
...
def main() -> None:
    ...
    cmd = [sys.executable, "ai/cp153_bm_1d_band_primary_save_run.py"]
    if args.force:
        cmd.append("--force")
