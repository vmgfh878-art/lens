...
def upsert_records(table: str, records: list[dict], on_conflict: str) -> None:
    if not records:
        return
    sanitized_records = [{key: _sanitize_value(value) for key, value in record.items()} for record in records]
    chunked_upsert(get_client(), table, sanitized_records, on_conflict=on_conflict)
