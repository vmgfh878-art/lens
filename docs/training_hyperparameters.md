...
### CP55-M 실행 하이퍼파라미터 및 제한 모드

CP55-M은 새 학습 없이 기존 후보를 통계 baseline과 비교했다. 긴 실행을 막기 위해 실행 모드는 아래처럼 고정한다.

| 모드 | 용도 | 계산 범위 |
|---|---|---|
| `--discover-only` | 후보 목록 수집 | baseline/forward/rolling 계산 없음 |
| `--smoke` | 경로 검증 | line 1개, band 1개, `smoke_limit_tickers=50`, minimal baseline |
| `--max-candidates 8` | 제한 재채점 | 대표 후보 8개, 후보별 timeout 300초, minimal baseline |

CP55 minimal baseline set은 아래처럼 제한한다.

| 역할 | baseline |
|---|---|
| line | `zero_line`, `momentum_line_horizon`, `reversal_line_horizon`, `random_or_shuffled_score_seed_0` |
| band | `constant_width_train_quantile`, `rolling_historical_quantile_band_w60`, `rolling_bollinger_return_band_w60_k1` |

최종 실행은 547초가 걸렸고 timeout 후보는 없었다. 가장 큰 병목은 `s60_q15_b2_direct_188::scalar_width` 200티커 band baseline 197.1초였다. 같은 split/q/role 조건은 baseline cache로 재사용하며, 실제로 `h5_dense_overlap`과 `tide_direct_original`은 cache hit로 즉시 처리됐다.

CP55 이후 band 실험의 기본 비교 기준은 “AI 후보 대 같은 조건의 best statistical baseline”이다. 새 학습을 시작하기 전에는 baseline-aware residual/scale band 후보를 먼저 설계한다.

### CP56-M 실험 전 계약 확인 기준

CP56-M 이후 새 학습 전에 확인할 계약 기준을 고정한다.

| 항목 | 기준 |
|---|---|
| feature_version | `v3_adjusted_ohlc` |
| target | line/band 모두 `raw_future_return` |
| checkpoint selector | line은 `line_gate`, band는 `band_gate`, 혼합은 `combined_gate` |
| deprecated selector | `coverage_gate`는 `combined_gate` alias로만 해석 |
| composite q 기준 | band checkpoint의 `q_low/q_high`를 nominal coverage 기준으로 사용 |
| role | `line_model`, `band_model`, `composite_model`을 config/meta에 명시 |
| composite sample | common ticker/asof_date/forecast_dates 수와 drop 비율 기록 |
| horizon check | 고정 5가 아니라 checkpoint `horizon`으로 series length 검증 |

P1 수정 전에는 full run이나 새 대형 sweep을 하지 않는다. 특히 `risk_first_lower_preserve` 정책 구현, `series_length_all_5`, ticker registry mismatch, scalar width center 계약, line-only run의 band output 저장 정책을 먼저 닫아야 한다.

### CP57-M 이후 실험 분리 기준

CP57-M 이후 composite 보정 정책은 Phase 1 본류 하이퍼파라미터가 아니다.

| 실험 계열 | 허용 기준 | 금지/제외 |
|---|---|---|
| line experiments | PatchTST/TiDE line 후보를 IC, spread, false-safe, severe recall로 평가 | band coverage로 line 탈락 판단 금지 |
| band experiments | CNN-LSTM/TiDE/PatchTST band 후보를 nominal calibration, interval score, dynamic width로 평가 | line_inside_band로 band 탈락 판단 금지 |
| overlay/product experiments | provenance, horizon/asof/forecast_dates mismatch 표시 | composite prediction을 모델 성능 단위로 사용 금지 |

`include_line_clamp`, `risk_first_lower_preserve`, `risk_first_upper_buffer_1.10`은 legacy policy로만 남긴다. 새 대형 실험은 line layer와 band layer를 따로 잡은 뒤 진행한다.

### CP59-P 제품 후보 노출 기준

학습/평가 하이퍼파라미터 기준에서 composite policy는 더 이상 Phase 1 제품 기본 후보가 아니다. `line_band_composite`와 `role=composite_model` run은 legacy demo/research artifact로만 보존하고, 제품 기본 latest completed 조회와 모델 학습 콘솔의 일반 completed 목록에서는 제외한다.

앞으로 모델 후보 비교는 line role과 band role을 분리해서 기록한다. overlay bundle, role별 inference meta, rolling AI band history, h5/h20 horizon 결정은 별도 CP에서 다룬다.

### CP60-M Composite Legacy Guard

Composite 정책은 Phase 1 본류 하이퍼파라미터가 아니다. `raw_composite`, `include_line_clamp`, `risk_first_lower_preserve`는 legacy 진단 정책으로만 유지하며, 새 line/band 모델 실험의 선택 기준이나 제품 기본 정책으로 사용하지 않는다.

`ai.composite_inference`에서 legacy composite row를 저장하려면 `--save`와 함께 `--allow-legacy-composite-save`를 명시해야 한다. 이 경로도 저장 meta에 deprecated/legacy 필드를 강제하므로, 본류 실험 기록에서는 line layer와 band layer를 독립 run으로 관리한다.

### CP61-M Metric Layer 기준

학습 checkpoint selector는 metric layer를 분리해서 해석한다. `line_gate`는 `line_metrics`만 우선 참조하고, `band_gate`는 `band_metrics`만 우선 참조한다. `combined_gate`와 `coverage_gate`는 deprecated 호환 모드이며, 새 모델 실험의 기본 선택 기준으로 쓰지 않는다.

기본 evaluation output에는 `line_inside_band_ratio`, `product_display_warning_rate` 같은 overlay 진단값을 포함하지 않는다. 해당 값이 필요한 연구용 진단에서는 `include_legacy_overlay_diagnostics=True`를 명시해 별도 `legacy_overlay_diagnostics` 블록으로만 본다.

### CP62-M Regrade 기준

기존 후보 재채점은 새 학습 없이 기존 metrics JSON을 사용한다. `ai/cp62_cp61_schema_candidate_regrade.py`는 CP53/CP54/CP55 산출물을 읽어 line 후보와 band 후보를 CP61 schema로 재분류하며, checkpoint forward나 DB 저장은 수행하지 않는다.

Line 판정은 `spearman_ic`, `long_short_spread`, false-safe, severe downside recall을 중심으로 보고, band 판정은 `coverage_abs_error`, `asymmetric_interval_score`, `band_width_ic`, `downside_width_ic`를 중심으로 본다. `coverage_gate`나 composite 정책은 CP62 후보 생존 판단에 사용하지 않는다.

### CP63-BM Feature Group Smoke 계획

CP63-BM 이후 band smoke는 새 feature contract 변경 없이 현 36개 입력의 부분집합으로만 시작한다. 공통 계약은 `feature_version=v3_adjusted_ohlc`, `target=raw_future_return`, `band_target_type=raw_future_return`, `role=band_model`, `checkpoint_selection=band_gate`다. `coverage_gate`와 `combined_gate`는 사용하지 않는다.

| 실험 | feature_set | epochs | limit_tickers | W&B | 이유 |
|---|---|---:|---:|---|---|
| CNN-LSTM seq60 q15-b2 direct | `price_volatility` | 3 | 50 | off 권장 | 기존 s60 q15-b2 direct의 dynamic width 신호를 market/fundamental noise 없이 재확인 |
| CNN-LSTM seq60 q20-b2 direct | `price_volatility` | 3 | 50 | off 권장 | q15 대비 coverage/width trade-off 확인 |
| TiDE q10-b2 param | `technical_only` | 3 | 50 | off 권장 | param width watch 후보가 feature pruning으로 interval_score를 개선하는지 확인 |
| TiDE q10-b2 direct | `price_volatility` | 3 | 50 | off 권장 | fail에 가까운 direct 구조의 하한 검증 |

위 smoke는 학습 지시가 별도로 있을 때만 실행한다. 이번 CP에서는 학습을 돌리지 않았고 save-run도 사용하지 않았다. 통계 baseline은 Bollinger, rolling quantile, constant train quantile, volatility scaled constant band를 비교 기준으로만 둔다. AI residual/scale 보정 모델은 Phase 1 BM 본류가 아니며, baseline-aware 보정은 Phase 1.5 후보로만 기록한다.

### CP64-BM Feature Group Smoke 실행값

CP64-BM은 CP63 계획을 실제 smoke로 실행했다. 공통 조건은 `feature_version=v3_adjusted_ohlc`, `role=band_model`, `band_target_type=raw_future_return`, `checkpoint_selection=band_gate`, `limit_tickers=50`, `epochs=3`, `batch_size=256`, `wandb=false`, `save-run=false`다. full 473 ticker, DB 쓰기, UI/backend 수정, composite/overlay 지표, 통계 baseline 보정 구현은 수행하지 않았다.

| 실험 | feature_set | n_features | 상태 | 실행 초 | epoch 평균 초 | 판정 |
|---|---|---:|---|---:|---:|---|
| CNN-LSTM seq60 q15-b2 direct | `price_volatility` | 10 | PASS | 198.9 | 45.35 | `band_watch` |
| CNN-LSTM seq60 q20-b2 direct | `price_volatility` | 10 | PASS | 186.8 | 43.10 | `band_survive` |
| CNN-LSTM seq60 q15-b2 direct | `price_volatility_volume` | 11 | PASS | 257.3 | 63.67 | `band_survive` |
| CNN-LSTM seq60 q15-b2 direct | `no_fundamentals` | 30 | PASS | 195.5 | 45.59 | `band_watch` |
| TiDE q10-b2 param | `technical_only` | 11 | PASS | 113.2 | 20.22 | `band_survive` |
| TiDE q10-b2 direct | `price_volatility` | 10 | PASS | 119.9 | 20.16 | `band_watch` |

CNN-LSTM에서는 `price_volatility` q20과 `price_volatility_volume` q15를 다음 좁은 탐색 후보로 둔다. TiDE는 `technical_only` param이 smoke 생존권은 있으나 interval score가 약해 2순위 watch 후보로 둔다. `atr_ratio`와 `intraday_range_ratio`는 이번 hyperparameter matrix에 넣지 않았고, feature contract 변경이 필요한 별도 CP에서만 승격을 검토한다.

### CP65-LM PatchTST line feature set smoke 실행값

CP65-LM은 PatchTST line_model 전용 smoke다. 공통 조건은 `horizon=5 또는 20`, `seq_len=252`, `patch_len=32`, `patch_stride=16`, `ci_aggregate=target`, `checkpoint_selection=line_gate`, `target=raw_future_return`, `line_target_type=raw_future_return`, `limit_tickers=50`, `epochs=3`, `batch_size=256`, `wandb=off`, `no-compile`, `save-run=false`다.

| branch | feature_set | n_features | 상태 | 실행 초 | 판정 |
|---|---|---:|---|---:|---|
| h5 product line | `technical_only` | 11 | PASS | 73.7 | `line_watch` |
| h5 product line | `no_fundamentals` | 30 | PASS | 136.9 | `line_survive` |
| h5 product line | `price_volatility_volume` | 11 | REUSED | 73.7 | `line_watch` |
| h20 visual/risk | `technical_only` | 11 | PASS | 88.7 | `phase_1_5_visual_risk_watch` |
| h20 visual/risk | `no_fundamentals` | 30 | PASS | 150.8 | `visual_risk_survive` |
| h20 visual/risk | `price_volatility_volume` | 11 | REUSED | 88.7 | `phase_1_5_visual_risk_watch` |

`technical_only`와 `price_volatility_volume`은 현재 같은 column set이므로 같은 horizon 안에서는 중복 학습을 피하고 결과를 재사용했다. h5 `no_fundamentals`는 통계 baseline 기준 생존했지만 full_features h5 longer 대비 false-safe와 severe recall이 약해 주력 후보를 교체하지 않는다. h20 `no_fundamentals`는 기존 h20 full_features보다 false_safe_tail_rate가 낮고 severe_downside_recall이 높아 visual/risk branch 생존 후보로 둔다.

### CP64-D Indicator Backfill 기준

CP64-D는 학습 hyperparameter 변경이 아니라 indicator 데이터 계약 보수다. 모델 학습, save-run, checkpoint 생성은 실행하지 않았다.

백필 실행 조건은 `force_full_backfill=True`, `full_start_date=2015-01-01`, `lookback_days=60`, `timeframes=1D/1W/1M`이다. 실제 실행은 `backend.collector.jobs.compute_indicators.run()` 공식 경로를 batch 단위로 호출했다. 1D는 대형 batch 이후 일부 소배치 repair를 수행했고, 1W는 40 ticker 이하, 1M은 25 ticker 이하 batch로 완료했다.

BM 실험 관점에서 중요한 계약은 `atr_ratio`를 아직 training feature로 넣지 않는다는 점이다. `MODEL_N_FEATURES=36` 유지, `atr_ratio in MODEL_FEATURE_COLUMNS=False`다. 다음 BM에서 `atr_ratio`를 승격하려면 clipping/winsorization, feature_version bump, cache 재생성, checkpoint 호환성 분리를 먼저 정해야 한다.

`daily_range_ratio`와 `intraday_range_ratio`는 현재 indicator column이 아니므로 실험 feature set에 넣지 않는다. range 계열 후보는 현재 `high_ratio`와 `low_ratio`로만 평가하며, dedicated range ratio는 별도 feature contract CP 이후 사용한다.

### CP66-LM post-backfill h20 / 1W 실행값

CP66-LM 공통 실행 조건은 PatchTST `role=line_model`, `target=raw_future_return`, `line_target_type=raw_future_return`, `checkpoint_selection=line_gate`, `ci_aggregate=target`, `save-run=false`, W&B off, no-compile이다. GPU 실행은 `.venv\Scripts\python.exe`, `torch 2.11.0+cu128`, `NVIDIA GeForce RTX 5060 Ti`, `--device cuda`로 고정했다. CPU-only 시스템 Python은 사용하지 않는다.

1D h20 재검증은 아래 조건으로 수행했다.
...
| batch_size | 256 |
| feature_version | `v3_adjusted_ohlc` |
| source data hash | `f7c7b101` |

| feature_set | n_features | ic_mean | spread | false_safe_tail | severe_recall | fee_return | fee_sharpe | 판정 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| `full_features` | 36 | 0.0230 | 0.0067 | 0.2895 | 0.7120 | 1.9491 | 0.0763 | `visual_risk_survive` |
| `no_fundamentals` | 30 | 0.0214 | 0.0136 | 0.3361 | 0.6647 | 45.5232 | 0.1743 | `watch_default_display_forbidden` |
| `technical_only` | 11 | 0.0222 | 0.0063 | 0.3237 | 0.6783 | 1.7153 | 0.0736 | `watch_default_display_forbidden` |

h20 제품 표시 기준은 false_safe_tail_rate 0.30 미만을 기본 표시 최소선으로 둔다. CP66에서는 `full_features`만 이 기준을 통과했으며, h20은 h5 기본 후보가 아니라 중기 line / visual-risk branch로 기록한다.

1W readiness 및 선택 smoke는 아래 조건으로 수행했다.
...
| limit_tickers | 50 |
| epochs | 1 |
| source data hash | `ca721eb6` |
| readiness eligible ticker | 447 |
| split gap | h_max=12 |

1W readiness는 feature/target NaN/Inf 0, split gap 통과, future label leakage blocker 없음으로 PASS다. 1W smoke는 completed였지만 ic_mean -0.0144, spread 0.0067, false_safe_tail_rate 0.3424, severe_downside_recall 0.6521이라 제품 후보가 아니라 readiness smoke로만 기록한다. 다음 1W 확대 전에는 default ticker registry stale 신호를 먼저 정리한다.

### CP67-LM h20 100티커 실행값

CP67-LM은 CP66 h20 `full_features`의 100티커 재현성 확인만 수행했다. 공통 조건은 PatchTST `role=line_model`, `timeframe=1D`, `horizon=20`, `seq_len=252`, `patch_len=32`, `patch_stride=16`, `checkpoint_selection=line_gate`, `target=raw_future_return`, `line_target_type=raw_future_return`, `batch_size=256`, `epochs=3`, `device=cuda`, `amp_dtype=bf16`, W&B off, no-compile, save-run=false다.
...
| ratio p99 sanity | PASS |

| 실험 | feature_set | n_features | ic_mean | spread | false_safe_tail | severe_recall | h11_h20 false_safe | h11_h20 recall | fee_sharpe | 판정 |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| h20_full_features_post_backfill_100 | `full_features` | 36 | 0.0667 | 0.0280 | 0.3918 | 0.6190 | 0.3829 | 0.6295 | 0.4219 | `fail` |

100티커에서는 IC/spread/fee 계열은 강해졌지만 line의 보수성 기준이 무너졌다. CP67 판정 기준상 false_safe_tail_rate 0.35 이상, severe_downside_recall 0.65 미만, h11_h20 false_safe_tail_rate 0.35 이상은 제품 표시 금지 조건이다. 따라서 A가 fail이므로 `h20_no_fundamentals_post_backfill_100`은 실행하지 않았다. 선택 5epoch도 A가 product_aux_pass가 아니라 실행하지 않았다.

다음 h20 실험은 같은 구조의 단순 100티커 반복보다 false-safe를 낮추는 후보를 별도 설계해야 한다. 제품 기본 표시 하이퍼파라미터 관점에서는 h5 단기 line만 기본 ON으로 유지한다.

### CP68-LM h20 post-hoc conservative calibration 실행값

CP68-LM은 새 학습이 아니라 CP67 checkpoint 기반 post-hoc calibration이다. 실행 checkpoint는 `ai/artifacts/checkpoints/patchtst_1D_patchtst-1D-8a5e01c0f891.pt`이고, 공통 조건은 PatchTST `role=line_model`, `timeframe=1D`, `horizon=20`, `seq_len=252`, `patch_len=32`, `patch_stride=16`, `feature_set=full_features`, `target=raw_future_return`, `line_target_type=raw_future_return`, `limit_tickers=100`, `batch_size=256`, `device=cuda`, `amp_dtype=bf16`이다. 학습 epoch는 추가로 돌리지 않았고 W&B off, save-run=false다.
...
| eligible_tickers | 93 |
| feature/target NaN/Inf | 0 / 0 |
| offset fit split | validation |
| evaluation split | test |
| target false_safe_tail_rate | < 0.30 |

| 정책 | fit 방식 | test ic_mean | test spread | test false_safe_tail | test severe_recall | test upside_sacrifice | 판정 |
|---|---|---:|---:|---:|---:|---:|---|
| raw_line | none | 0.0667 | 0.0280 | 0.3918 | 0.6190 | 0.1104 | FAIL |
| global_downshift | validation offset 0.0157249 | 0.0667 | 0.0280 | 0.2756 | 0.7258 | 0.1261 | PASS |
| horizon_bucket_downshift | h1_h5 0.0113019 / h6_h10 0.0138117 / h11_h20 0.0148215 | 0.0667 | 0.0280 | 0.2857 | 0.7171 | 0.1247 | PASS |
| volatility_scaled_downshift | validation base offset 0.0155880 | 0.0632 | 0.0256 | 0.2576 | 0.7474 | 0.1316 | PASS |

best 정책은 `horizon_bucket_downshift`다. `global_downshift`도 PASS지만 horizon별 안전 오판을 따로 맞추는 bucket 방식이 h20 display calibration 의미에 더 맞고, volatility_scaled 방식보다 IC/spread/fee 희생이 작다. 이 calibration은 성능 부풀리기가 아니라 conservative display calibration으로 기록하며, test를 보고 offset을 선택하지 않는다.

### CP69-LM line 보수성 계약 감사 결과

line 학습 보수성의 기준 하이퍼파라미터는 `AsymmetricHuberLoss(alpha=1, beta=2, delta=1)`이다. 손실의 error 정의는 `prediction - target`이며, error가 양수인 과대예측에 `beta=2`가 적용된다. target 1.0 기준 prediction 2.0의 loss는 1.0, prediction 0.0의 loss는 0.5로 확인했다.

감사 대상 checkpoint config의 핵심 값은 다음과 같다.

| 후보군 | alpha | beta | delta | lambda_line | checkpoint_selection | line_target_type | 판정 |
|---|---:|---:|---:|---:|---|---|---|
| h5 longer / baseline / dense | 1.0 | 2.0 | 1.0 | 1.0 | `line_gate` | `raw_future_return` | `verified_trained_conservative` |
| CP65 h20 후보 | 1.0 | 2.0 | 1.0 | 1.0 | `line_gate` | `raw_future_return` | `verified_trained_conservative` |
| CP67 h20 full 100 | 1.0 | 2.0 | 1.0 | 1.0 | `line_gate` | `raw_future_return` | `verified_trained_conservative` |
| CP68 bucket downshift | 1.0 | 2.0 | 1.0 | 1.0 | `line_gate` | `raw_future_return` | `posthoc_only_not_training` |

CP68의 offset 정책은 학습 하이퍼파라미터가 아니라 표시 보정 파라미터다. 이후 문서에서는 checkpoint 원출력 line을 `raw_model_line`, alpha/beta loss로 학습된 line을 `trained_conservative_line`, validation offset으로 낮춘 표시선을 `display_calibrated_line`으로 구분한다.

### CP70-LM h20 display calibration 정책 비교 실행값

CP70-LM은 새 학습이 아니라 CP67 checkpoint 기반 CPU-only forward-only 재평가다. 실행 조건은 `device=cpu`, `amp_dtype=off`, `batch_size=256`, `limit_tickers=100`, `timeframe=1D`, `horizon=20`, `seq_len=252`, `patch_len=32`, `patch_stride=16`이다. GPU는 쓰지 않았고 VRAM 사용량은 해당 없음이다. 실행 시간은 약 424.8초다.

주의: current source hash는 `3ac43945`로 CP67/CP68의 `f7c7b101`과 다르다. 따라서 아래 값은 CP67 checkpoint를 현재 100티커 cache/data에 적용한 정책 안정성 확인이며, CP68 동일-hash 재현값은 아니다.

| 정책 | line_type | ic_mean | spread | false_safe_tail | false_safe_severe | severe_recall | upside_sacrifice | fee_sharpe | 판정 |
|---|---|---:|---:|---:|---:|---:|---:|---:|---|
| raw_model_line | `raw_model_line / trained_conservative_line` | 0.0676 | 0.0284 | 0.3911 | 0.3804 | 0.6196 | 0.1107 | 0.4271 | fail |
| global_downshift | `display_calibrated_line` | 0.0676 | 0.0284 | 0.2744 | 0.2731 | 0.7269 | 0.1265 | 0.4271 | default_off_candidate |
| horizon_bucket_downshift | `display_calibrated_line` | 0.0676 | 0.0284 | 0.2841 | 0.2814 | 0.7186 | 0.1251 | 0.4271 | default_off_candidate |
| volatility_scaled_downshift | `display_calibrated_line` | 0.0640 | 0.0259 | 0.2566 | 0.2516 | 0.7484 | 0.1320 | 0.3779 | default_off_candidate |

선택 정책은 `horizon_bucket_downshift`다. `global_downshift`는 가장 단순한 fallback이고, `volatility_scaled_downshift`는 false-safe는 가장 낮지만 IC/spread/fee 희생과 validation scale 과적합 가능성 때문에 우선하지 않는다. h20 표시선은 제품 기본 ON이 아니라 기본 OFF / 사용자 선택형 중기 참고선 후보로만 유지한다.

### CP74-G W&B 실행 정책

full training에서 `--wandb` 기본값은 유지하되 W&B는 optional이다. `WANDB_API_KEY`가 현재 프로세스에 없거나 `.env` 자동 로드가 실패하거나 `wandb.init()`이 예외를 내도 학습 hyperparameter와 training loop는 중단하지 않는다. 이 경우 `wandb_status`를 `disabled_missing_key`, `package_missing`, `disabled_by_env`, `disabled_init_failed` 중 하나로 남긴다.

`--no-wandb`는 W&B를 완전히 끈다. 이 경우 status는 `disabled_by_cli`이며 full training 결과와 save-run config에는 W&B run id가 비어 있어야 한다. `online_ok`일 때만 `wandb_run_id`와 `wandb_run_url`을 기록한다.

sweep은 다르다. `ai.sweep --wandb`는 required로 간주하고 trial 호출 시 `wandb_required=True`를 넘긴다. W&B 없이 sweep을 실행하려면 `--no-wandb`를 명시한다. 이 정책은 full candidate 단일 학습이 W&B 때문에 막히는 문제를 풀면서도 sweep 추적이 조용히 사라지는 문제를 막기 위한 것이다.

### CP75-G Local Training Progress Logging

full training의 local logging 기본값은 on이다.

| 항목 | 값 |
|---|---|
| 기본 경로 | `logs/runs/{run_id}/` |
| epoch metrics | `metrics.jsonl` |
| 최종 요약 | `summary.json` |
| 실행 config | `config.json` |
| 비활성화 | `--no-local-log` |
| 경로 변경 | `--local-log-dir <path>` |

`metrics.jsonl`은 epoch마다 `run_id`, `epoch`, `epochs`, `elapsed_seconds`, `epoch_seconds`, `estimated_remaining_seconds`, `learning_rate`, `train_total_loss`, `val_total_loss`, `val_forecast_loss`, `best_so_far`, `epochs_since_improve`, `checkpoint_selection`, `gate_status`, `vram_peak_allocated_mb`, line metric, band metric을 기록한다.

`summary.json`은 `status`, `model`, `timeframe`, `horizon`, `feature_set`, `source_data_hash`, `checkpoint_path`, `best_metrics`, `test_metrics`, `wandb_status`, `total_elapsed_seconds`를 기록한다. 현재 source hash는 DatasetPlan에 직접 실려 있지 않아 nullable이며, cache manifest 개선 CP에서 보강한다.

이 변경은 관측성 추가이며 W&B 정책, loss weight, batch size, split, target, checkpoint selector, save-run schema를 바꾸지 않는다.

### CP72-BM 1D Band Full Product Candidate

CP72-BM 최종 학습 조건은 다음과 같다.

| 항목 | 값 |
|---|---|
| run_id | `cnn_lstm-1D-d0c780dee5e8` |
| model / role | `cnn_lstm` / `band_model` |
...
| device / amp_dtype | `cuda` / `bf16` |
| fp32_modules | `lstm,heads` |
| compile | `--no-compile` |
| W&B | `--no-wandb`, `WANDB_MODE=disabled` |
| save_run | true |
| num_workers | 0 |

실행은 Windows background wrapper 없이 foreground PowerShell에서 `.venv\Scripts\python.exe`를 직접 호출했다. CUDA 확인 결과는 `.venv` Python, `torch 2.11.0+cu128`, `cuda=True`, `NVIDIA GeForce RTX 5060 Ti`였다.

최종 test band 결과는 nominal_coverage 0.7000, empirical_coverage 0.7141, coverage_abs_error 0.0141, lower_breach_rate 0.1499, upper_breach_rate 0.1359, avg_band_width 0.0607, asymmetric_interval_score 0.1244, band_width_ic 0.3724, downside_width_ic 0.0673이다.

CP72 중 `band_gate`의 legacy fixed target 문제를 고쳤다. q15/q85 band는 nominal_coverage가 0.70이므로 gate는 고정 0.85가 아니라 metric에 저장된 nominal coverage와 tail breach error를 기준으로 본다.
### CP75-LM 1D h5 line product candidate hyperparameters

CP75-LM h5 line 저장 후보 학습 조건은 다음과 같다.

| 항목 | 값 |
|---|---|
| run_id | `patchtst-1D-efad3c29d803` |
| model / role | `patchtst` / `line_model` |
| timeframe / horizon | `1D` / 5 |
| seq_len | 252 |
| patch_len / patch_stride | 32 / 16 |
| feature_set | `full_features` |
| n_features | 36 |
| target / line_target_type | `raw_future_return` / `raw_future_return` |
| checkpoint_selection | `line_gate` |
| alpha / beta / delta | 1.0 / 2.0 / 1.0 |
| lambda_line / lambda_band / lambda_cross | 1.0 / 2.0 / 1.0 |
| lambda_width / lambda_direction | 0.1 / 0.1 |
| band_mode | `direct` |
| epochs / batch_size | 5 / 256 |
| device / amp_dtype | `cuda` / `bf16` |
| compile | `--no-compile` |
...
| local_log_dir | `docs/cp75_lm_1d_h5_full_line_product_candidate_logs` |

사전 실행 환경은 `.venv\Scripts\python.exe`, `torch 2.11.0+cu128`, CUDA 사용 가능, `NVIDIA GeForce RTX 5060 Ti`였다. 예상치는 70-140분, 3-6GB VRAM으로 기록했고 실제 학습은 약 2,816초, peak allocated VRAM 3,030.66MB였다.

사전 게이트는 source_data_hash `3ac43945`, CP72 BM 동일 hash, feature_version `v3_adjusted_ohlc`, MODEL_FEATURE_COLUMNS 36개, feature/target NaN/Inf 0, ratio sanity PASS, `no --limit-tickers` 기준 eligible 전체 473개로 통과했다. `atr_ratio`는 모델 입력에 포함하지 않았다.

test line_metrics 주요 결과는 ic_mean 0.0406, ic_ir 0.2080, ic_t_stat 4.0387, long_short_spread 0.0063, spread_ir 0.1916, spread_t_stat 3.7306, direction_accuracy 0.5027, false_safe_tail_rate 0.3931, false_safe_severe_rate 0.3942, severe_downside_recall 0.6058, conservative_bias -0.0119, upside_sacrifice 0.0585, mae 0.0487, smape 1.4855, fee_adjusted_return 6.1278, fee_adjusted_sharpe 0.1749다. 순위/수익 신호는 양수이나 false-safe가 높아 `completed_line_watch`로 둔다.

## 2026-05-03 제품 모델 현황 및 표시 기준

현재 제품 화면에서 직접 사용하는 1D 모델은 line layer 1개와 band layer 1개다. 두 모델은 composite로 합치지 않는다.
...
| target | `raw_future_return` |
| checkpoint_selection | `line_gate` |
| loss | alpha 1.0, beta 2.0, delta 1.0 |
| epochs / batch_size | 5 / 256 |
| device / amp | CUDA / bf16 |
| W&B | disabled_by_cli |
| 저장 상태 | completed, predictions/evaluations 저장 완료 |

주요 line metric은 `ic_mean=0.0406`, `long_short_spread=0.0063`, `fee_adjusted_sharpe=0.1749`다. 순위/수익 방향 신호는 양수라 제품 화면에 표시할 근거가 있다. 다만 `false_safe_tail_rate=0.3931`, `false_safe_severe_rate=0.3942`, `severe_downside_recall=0.6058`이라 위험 회피선으로는 아직 약하다. 따라서 제품 설명에서는 “보수적 예측선 v1”로 보여주되, 내부 판정은 `completed_line_watch`로 둔다.
...
주요 band metric은 `empirical_coverage=0.7141`, `coverage_abs_error=0.0141`, `lower_breach_rate=0.1499`, `upper_breach_rate=0.1359`, `avg_band_width=0.0607`, `asymmetric_interval_score=0.1244`, `band_width_ic=0.3724`, `downside_width_ic=0.0673`다. q15/q85 밴드의 목표 포함률 0.70에 가깝고, 폭이 변동성에 반응하는 신호가 있어 1D AI 밴드 v1로 제품 화면에 연결한다.

### 제품 화면 평가 표현 기준

AI 모델 화면의 기본 평가는 raw metric 나열이 아니라 다음 구조를 따른다.

| 항목 | 의미 |
|---|---|
| 목표 | 이 모델이 통과해야 하는 기준 |
| 실제 | 저장 run의 실제 평가값 |
| 차이 | 목표 대비 부족하거나 초과한 정도 |
| 판정 | 제품 표시, watch, 개선 필요 등 |
| 해석 | 일반 사용자도 이해할 수 있는 자연어 설명 |

이전 실험은 제품 모델과 분리한다. 예측선 실험과 밴드 실험으로만 묶고, 사용자가 열었을 때만 보여준다. NaN 실패, 계산 불가, composite 실험은 기본 UI에서 숨긴다. 품질 기준 미달 실험은 “왜 기준에 못 미쳤는지”가 metric으로 설명 가능한 경우에만 보여준다.

### 관측성 기준

full training은 W&B 없이도 돌아가야 한다. `--no-wandb` 또는 `WANDB_MODE=disabled`일 때는 `wandb_status=disabled_by_cli`로 기록한다. sweep처럼 W&B가 꼭 필요한 작업만 required로 둔다.

학습 진행 추적은 local log를 기본으로 사용한다. 기본 경로는 `logs/runs/{run_id}/`이고, CP75 이후 `metrics.jsonl`, `summary.json`, `config.json`을 남길 수 있다. 제품 후보 full run은 가능하면 `--local-log-dir docs/<cp>_logs`처럼 CP별 로그 경로를 명시한다.

### 다음 실험 메모

- line layer는 1D h5 제품 후보가 있으나 false-safe 개선이 남았다.
- band layer는 1D h5 제품 후보가 있으나 1W band는 아직 없다.
- h20 line은 시각적으로 매력적이지만 Phase 1 제품 기본에서 제외한다. 현재는 Phase 1.5 visual/risk branch다.
- 백테스트 전략은 현재 모델 하이퍼파라미터 평가가 아니라 제품 UX 데모다. Trend Guard v1과 Risk Guard v1은 나중에 더 나은 전략 설계로 교체할 수 있다.

### CP86-D yfinance 로컬 primary 수집 파라미터

CP86-D는 모델 학습 하이퍼파라미터를 바꾸지 않았다. `MODEL_FEATURE_COLUMNS=36`, `MODEL_N_FEATURES=36`, feature contract `v3_adjusted_ohlc`를 유지했고, 모델 학습, live inference 연결, 1W/1M 모델 실험은 실행하지 않았다.

시장 데이터 수집 기본 운영값은 로컬 기준 `MARKET_DATA_PROVIDER=yfinance`, `MARKET_DATA_FALLBACK_PROVIDER=eodhd`로 둔다. yfinance 호출은 `auto_adjust=False`, `actions=False`, `threads=False`로 고정해 raw OHLC와 `Adj Close`를 함께 보존하고, 저장 전 `adjusted_factor = adjusted_close / close`로 adjusted OHLC sanity gate를 통과해야 한다.

CP86 dry-run 비교 범위는 `2024-05-03`부터 `2026-05-03`까지 최근 2년이며, 비교 티커는 `AAPL`, `MSFT`, `NVDA`, `TSLA`, `AMZN`, `GOOGL`, `META`, `NFLX`, `AVGO`, `AMD`, `SPY`, `QQQ`다. DB write는 수행하지 않았고, 이후 제한적 write를 할 때는 `--write`를 명시한 뒤 indicators 재계산과 data quality check를 이어서 실행해야 한다.
### CP96-D yfinance 전환 전 데이터 가드

CP96-D는 학습 hyperparameter를 바꾸지 않았다. 모델 학습, save-run, live inference, product run 교체도 실행하지 않았다.
...
| `MODEL_N_FEATURES` | 36 |
| yfinance smoke/전환 기본 provider | `MARKET_DATA_PROVIDER=yfinance` |
| EODHD legacy read | `source='eodhd'` 또는 null |
| yfinance read | `source='yfinance'` |
| 1W/1M resample | 완성 period만 사용 |

학습 전 cache 준비 단계에서 `source_data_hash`는 indicator value checksum을 포함한다. CP95 yfinance 100티커 cache는 기존 `5be36437`에서 CP96 계약 기준 `b6ad28de`로 바뀌므로, 다음 학습 또는 feature build 때 새 cache가 만들어져야 한다.

다음 전체 universe yfinance write 이후 학습 smoke를 다시 할 때는 기존 CP95 smoke 조건을 유지하되, cache hash가 새 provider/source/checksum 계약을 반영하는지 먼저 확인한다.

### CP116-S Windows torch import 운영값

학습/추론 entrypoint는 `ai.torch_bootstrap.bootstrap_torch()`를 통해 torch를 먼저 로드한다. 기본 환경값은 `PYTHONUTF8=1`, `KMP_DUPLICATE_LIB_OK=TRUE`, `TORCHDYNAMO_DISABLE=1`, `PYTORCH_NVML_BASED_CUDA_CHECK=1`이다.

CUDA 학습에서는 `CUDA_VISIBLE_DEVICES=-1`을 설정하지 않는다. CPU-only 제품 리허설 또는 CPU-only runner에서만 `bootstrap_torch(cpu_only=True)`를 사용한다. 이 변경은 학습 hyperparameter가 아니라 Windows native DLL 안정화 운영 계약이다.

### CP124-BM 1W band downside guard 손실 파라미터

CP124-BM에서 band loss의 lower/upper quantile 가중치를 선택적으로 조절하는 CLI를 추가했다. 기본값은 `--lower-band-loss-weight 1.0`, `--upper-band-loss-weight 1.0`이며, 이 경우 기존 `ForecastCompositeLoss` 동작과 같다. CP124 실험에서 lower guard는 lower weight 1.5, asymmetric guard는 lower weight 2.0과 upper weight 1.0으로 실행했다.

공통 실험 조건은 `timeframe=1W`, `horizon=4`, `seq_len=104`, `epochs=3`, `batch_size=128`, `checkpoint_selection=band_gate`, `band_target_type=raw_future_return`, `WANDB_MODE=disabled`, `save_run=false`였다. `tide_pvv_q15_param`은 `feature_set=price_volatility_volume`, `q_low/q_high=0.15/0.85`, `band_mode=param`, `lambda_band=2.0`이고, `cnn_full_q10_direct`는 `feature_set=full_features`, `q_low/q_high=0.10/0.90`, `band_mode=direct`, `lambda_band=2.0`이다.

`lambda_width`는 계속 레거시 호환 인자이며 현재 손실 계산에는 쓰지 않는다. 따라서 width alignment 강화는 CP124에서 실행하지 않고 별도 loss 설계 필요 항목으로 남겼다.
### Phase 1 모델 실험 운영 기준

Phase 1의 다음 모델 실험은 네 슬롯의 500티커 v1을 만드는 데 집중한다. 실험별 세부 하이퍼파라미터 matrix는 각 실험 계획서에서 관리하고, 이 공용 문서에는 전체 운영 기준만 남긴다.

공통 고정 기준은 다음과 같다.

| 항목 | 기준 |
|---|---|
| 학습 기준 데이터 | EODHD 500 local parquet |
| feature contract | `v3_adjusted_ohlc` |
| 1D absolute minimum | 450 rows |
| 1W absolute minimum | 78 rows |
| 1M | Phase 1 모델 실험 제외 |
| line 제품 후보 | `checkpoint_selection=line_gate` 필수 |
| band 제품 후보 | `checkpoint_selection=band_gate` 필수 |
| `val_total` completed run | 제품 후보 자동 승격 금지 |
| calibration | 실제 inference 저장에 적용되지 않으면 제품 성능으로 말하지 않음 |

W&B/Optuna는 coarse 비교 후 살아남은 후보에만 좁게 사용한다. 단일 seed sweep 결과는 최종 결론이 아니며, 제품 v1 확정 전에는 seed 3~5개 안정성 확인을 거친다. 저장과 프론트 연결은 네 슬롯 후보가 정리된 뒤 별도 CP에서 진행한다.
