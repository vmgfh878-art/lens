...
### 재검토 (2026.06) — v1 제출용 "서빙 도커"는 분리 트랙
학습 재현 도커(GPU, cu128·sm_120)는 위 ROI 논리대로 **v2 유지**(base image 잠그기 까다로움 + 마감 리스크). 다만 **6/21 제출/채점자 재현** 관점은 결론이 갈린다:
- Docker 는 **매니페스트를 대체하지 않음** — 매니페스트=lineage(무엇으로 만들었나), Docker=runtime(어떤 환경에서 도나). 다른 축이라 매니페스트 작업은 유효.
- **운영 배포는 이미 git push → Render 자동**. 도커로 바꿔도 안 편해지고 512MB 제약 그대로. "도커=배포 편함" 통념은 이 프로젝트엔 거의 무효.
- 진짜 이득 = 채점자가 `docker run` 한 줄로 API 기동(재현 편의 + 포트폴리오 신호).
- **결론: v1엔 CPU 서빙/추론 Dockerfile**(backend + 45MB serving parquet → `docker run` → API)까지가 저위험·고효용. **GPU 학습 도커는 v2.**

---

## 3.5 전략 백테스트 엔진 개선 (편향 직면과 별개)

생존편향/look-back(§ HTML ④, backtest_single_ticker_strategy_v2_plan.md)과 **별개로 엔진 자체**를 키운다.
현재 계약 `single_ticker_long_cash_average` — 단일 종목 long/cash 평균이라 "전략"이라기엔 얇다.
- ① **포트폴리오 백테스트** — N종목 동시 보유·리밸런싱·자본 배분
- ② **포지션 사이징** — 고정 비중 → 변동성·신뢰도 기반
- ③ **슬리피지·체결·유동성** 모델 — 현재는 거래비용만 분리
- ④ **전략 walk-forward 검증** — 룩백 파라미터 최적화 → 분리 구간 검증(과적합 차단)
- 차별화(§4.4 LLM, ⑨ 결합): "AI 밴드 하단 이탈 → 청산 필터"가 이 엔진 위에서 동작.
