...
CP252 검증 방어 전략을 운영 경로(`_raw_target`)로 **byte-identical 포팅**해 라이브 등록(+대조군 유지),
구 `ai_band_defense_v2` 교체, CP248~251(v1) ablation 코어·데이터·문서 제거. **AI는 수익엔 기여 못함을
정직히 표기**하고 방어 전용으로만 노출. 회귀 0, RSS 327MB(<512). (후속: 참여 6%로 화면에 볼 게 없던
`lineband_defense` 는 사용자 결정으로 제거 → 라이브 방어 2 + 대조군.)

## 1. 라이브 등록 — 신규 방어 2 + 대조군
...
| `indicator_balance_v2` | 지표 균형 | no-AI 대조군 (유지) | — | 61% | — |

- 후속 제거: `lineband_defense`(눌림목+라인모멘텀+밴드)는 held-out 방어 최강(+7.8%)이나 참여율 6%로
  거의 현금만 들고 있어 화면에서 볼 게 없음 → 제거(사용자 결정). 남은 둘은 ~23-25% 참여로 viewable.
- 이름 = 사용 지표 노출(괄호 AI 표기 금지, 사용자 확정). 배지 없음·정직 카피는 카드 설명/범위주(사용자 확정).
- ★정직(§1.1): 헤드라인 ΔMaxDD는 **위험 종목 회피(현금)** 비중이 큼. 평균 참여율 23~25%(현금 多) =
  보수적 "리스크 가드". 카드에 "수익엔 기여 못함(공격형 OOS null·상승장 caveat), 위험할 때 현금으로
