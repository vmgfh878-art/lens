...
## 작업 방식
- 큰 오더(다단계·되돌리기 어렵거나 범위 넓은 작업)는 착수 전, 모호하거나 판단이 갈리는 지점이 있으면 추측으로 밀지 말고 먼저 묻고 진행한다.
- 코딩 지시는 직접 구현하고 같은 턴에 자가 리뷰(정합·구조 결함·영향)를 보고한다. 오케스트레이터/구현자 분리 안 함.
- 프론트 표현 결정(섹션 제목·배지·톤·카피)은 자가 해석 금지 — AskUserQuestion 으로 먼저 묻는다.
- 사용자 의견이 틀리면 동조·아부 말고 근거로 반박한다.
- 커밋·push 는 사용자가 요청할 때만. push 는 주요 파일(코드·보고서·서빙 데이터)만 origin; CP 지시서·리포트·내부 plan(docs/)은 로컬 유지(번호만 이어감).
- 문서·README 에 마크다운 별표(**) 강조·이모티콘 금지. 문서·장기기록 파일은 Write 1회 후 Edit(전체 rewrite 금지), 존재 확인은 Grep.
- 문제 해결은 항상 근본 원인 제거가 목적. 증상만 가리는 임시방편(개별 제외·allowlist·우회)은 그 자체로 끝내지 말고, 같은 클래스 문제가 재발 안 하게 구조적으로 고치는 걸 우선한다(예: 특정 티커 하나 예외처리보다 그 티커를 걸리게 만든 로직 자체 수정).
