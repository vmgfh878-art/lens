/**
 * CP220 — 1W Band (CP178 TiDE WFLOCK) 전용 실험 매니페스트 아카이브.
 * 위치: SignificanceSection 다음. 기본 접힘 (details).
 * 사용 데이터 (feature pack / 전처리) + 재현 매니페스트 (환경 / fold / 산출물 / 절차 / TODO) 한 덩어리.
 *
 * 단일 진리: docs/v1_operating_models_reproducibility.md 섹션0 (환경) + 섹션3 (1W 밴드).
 */
import { USAGE_DATA } from "@/lib/training/usageData";
import { REPRODUCIBILITY } from "@/lib/training/reproducibility";

export default function Band1wExperimentArchive() {
  const usage = USAGE_DATA["band-1w"];
  const repro = REPRODUCIBILITY["band-1w"];
  if (!usage || !repro) {
    return null;
  }
  return (
    <details className="experiment-archive">
      <summary>
        <span className="experiment-archive__title">실험 매니페스트 — 1W Band · CP178</span>
