/**
 * CP220 — Line (CP210) 전용 실험 매니페스트 아카이브.
...
export default function LineExperimentArchive() {
  const usage = USAGE_DATA["line-1d"];
  const repro = REPRODUCIBILITY["line-1d"];
  if (!usage || !repro) {
    return null;
  }
  return (
    <details className="experiment-archive">
      <summary>
        <span className="experiment-archive__title">실험 매니페스트 — Line · CP210</span>
        <span className="experiment-archive__meta-line">
          {repro.backbone} · run_id <code>{repro.runId}</code> · seeds [{repro.seeds.join(", ")}] · {repro.folds.length}-fold walk-forward
        </span>
      </summary>
