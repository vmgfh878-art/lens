...
export const STRATEGIES: StrategyDefinition[] = [
  ...
  },
  {
    ...
    scopeNote:
      "AI 라인·밴드를 이 종목 기준 '위험-상태 게이지'로 사용해, 위험 국면에서만 시장 노출을 줄여 낙폭과 대형손실을 줄이는 보수적 방어입니다. 시장 참여 비중이 낮아 수익률 자체는 단순 보유보다 낮습니다. 검증은 티커 분할(개발 200 / 검증 271, 겹침 0) + 각 티커 기간 분할(앞 70% 탐색 / 뒤 30% 검증) out-of-sample + 다중비교 보정으로 했습니다.",
    usesAi: true,
  },
  {
    id: "line_defense",
    ...
    shortLabel: "라인 방어",
    description:
      "밴드를 쓰지 않고 AI 라인만으로 위험 국면 진입을 거르는 방어 전략입니다. 모멘텀(20일 수익률·MACD 가속)으로 진입하되 AI 라인이 이 종목 기준 상위 분위(p60) 이상일 때만 들어갑니다. 시장 참여 비중이 낮아 수익률 자체는 단순 보유보다 낮고, 낙폭·대형손실 방어용입니다.",
    ruleRows: [
      ["전략 단위", "각 티커별 100% 보유 또는 100% 현금"],
    ...
    scopeNote:
      "밴드 없이 AI 라인을 이 종목 기준 상대 분위 게이트로 사용해 위험 국면 노출을 줄입니다. 시장 참여 비중이 낮아 수익에는 기여하지 않고 낙폭·대형손실 방어에 쓰입니다. 검증은 티커 분할(개발 200 / 검증 271) + 기간 분할(앞 70% / 뒤 30%) out-of-sample + 다중비교 보정으로 했습니다.",
    usesAi: true,
  },
];
